...
public class MAutobus extends BD {
    ...
    public ArrayList<Autobus> mostrarRutas() {
        try {
            ...
            while (r.next()) {
                ...
                a.setDestino(new Destino(r.getString("horario"), r.getInt("costo"), r.getString("destino")));
                ado.add(a);
            }
            return ado;
        } catch (SQLException ex) {
    ...
    public void reservarAsiento(int clave, int numero){
        try {
            ResultSet r = this.consultar("select idAutobus from autobus where clave=" + clave);
            while (r.next()) {
                this.modificar("capacidad", "estado=1", "idAutobus=" +r.getInt("idAutobus")+ " and asiento="+numero);
            }
        } catch (SQLException ex) {
            Logger.getLogger(MAutobus.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    public int buscarAsiento(int clave, int numero)  {
        try {
            ResultSet r = this.consultar("select idAutobus from autobus where clave=" + clave);
            while (r.next()) {
                ResultSet r2 = this.consultar("select * from capacidad where idAutobus=" + r.getInt("idAutobus") + " and asiento=" + numero);
                r2.next();
                return r2.getInt("estado");
            }
